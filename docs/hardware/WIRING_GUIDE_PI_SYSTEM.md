...
Complete wiring guide for the Raspberry Pi 4B CAN hub with dual MCP2515 modules, ESP32-S3 display, and Arduino LED controller.
...
│  │                                                                     │    │
│  │  GPIO 8/7 (SPI CS)     → MCP2515 #1/#2 CS                          │    │
│  │  GPIO 10/9/11 (SPI)    → Both MCP2515 MOSI/MISO/SCLK               │    │
│  │  GPIO 25/24 (INT)      → MCP2515 #1/#2 INT                         │    │
│  │  GPIO 14/15 (UART)     → Arduino Nano RX/TX                        │    │
│  │  USB-A                 → ESP32-S3 USB-C                            │    │
│  │  Micro HDMI            → Pioneer Head Unit                         │    │
│  └────────────────────────────────────────────────────────────────────┘    │
│           │                        │                      │                 │
│           ▼                        ▼                      ▼                 │
│  ┌────────────────┐    ┌────────────────┐    ┌────────────────────────┐    │
...
│  │  D10/11/12/13    → MCP2515 SPI (CS/MOSI/MISO/SCK)                  │    │
│  └────────────────────────────────────────────────────────────────────┘    │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
```
   ...
   Pin 3:  MS-CAN High (125k) → Pi MCP2515 #2 CANH only
   Pin 5:  Ground             → Common ground for all devices
   Pin 6:  HS-CAN High (500k) → Pi MCP2515 #1 CANH + Arduino MCP2515 CANH
   Pin 11: MS-CAN Low (125k)  → Pi MCP2515 #2 CANL only
   Pin 14: HS-CAN Low (500k)  → Pi MCP2515 #1 CANL + Arduino MCP2515 CANL
   Pin 16: 12V Battery        → Buck converters (Arduino, Pi power)
```

### HS-CAN Sharing (Pi + Arduino)

Both the Pi and Arduino need to read the HS-CAN bus. Options:

**Option 1: Y-Splitter Cable (Recommended)**
- Use an OBD-II Y-splitter
- Pi connects to one branch, Arduino to the other
- Both read from the same bus

**Option 2: Parallel Tap**
- Splice both CAN modules to pins 6/14
- More complex wiring but single OBD connector

```
OBD-II Pin 6 (CANH) ──┬── Pi MCP2515 #1 CANH
                      └── Arduino MCP2515 CANH

OBD-II Pin 14 (CANL) ──┬── Pi MCP2515 #1 CANL
                       └── Arduino MCP2515 CANL
```
...
| MCP2515 (Pi) x2 | 3.3V | 0.1A | From Pi 3.3V rail |
| MCP2515 (Arduino) | 5V | 0.05A | From Arduino 5V |
