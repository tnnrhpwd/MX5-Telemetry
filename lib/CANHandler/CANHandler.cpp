...
bool CANHandler::runTwoArduinoTest() {
    // ============================================================================
    // TWO-ARDUINO CAN TEST
    // ============================================================================
    // Sends a test message to the Slave Arduino and waits for a response.
    // This verifies:
    //   - Master TX → CAN bus wiring
    //   - CAN bus physical layer (CANH/CANL connections)
    //   - Slave RX ← CAN bus wiring
    //   - Slave TX → CAN bus (response)
    //   - Master RX ← CAN bus
    //
    // PREREQUISITES:
    //   1. Slave must have CAN module connected (same wiring as Master)
    //   2. Slave must be in CAN test mode (send "CAN" command via USB to Slave)
    //   3. CANH connected to CANH, CANL connected to CANL between modules
    // ============================================================================
    
    Serial.println(F("\n=== TWO-ARDUINO CAN TEST ==="));
    Serial.println(F("Make sure Slave is in CAN test mode!"));
    Serial.println(F("(Send 'CAN' to Slave via USB first)"));
    ...
    // Step 1: Send test message to Slave (ID 0x123)
    unsigned char testData[8] = {0x4D, 0x41, 0x53, 0x54, 0x45, 0x52, 0x21, 0x00}; // "MASTER!"
    Serial.print(F("Sending to SLAVE ID=0x123... "));
    
    byte sendResult = can.sendMsgBuf(0x123, 0, 8, testData);
    if (sendResult != CAN_OK) {
        Serial.print(F("FAIL: send error="));
        Serial.println(sendResult);
        return false;
    ...
    // Step 2: Wait for response from Slave (ID 0x456)
    Serial.print(F("Waiting for SLAVE response... "));
    unsigned long startWait = millis();
    bool received = false;
    ...
    while (millis() - startWait < 2000) {  // 2 second timeout
        if (can.checkReceive() == CAN_MSGAVAIL) {
            ...
            // Check if it's the expected response from Slave
            if (rxId == 0x456) {
                Serial.println(F("*** SLAVE RESPONSE RECEIVED! ***"));
                
                // Check for ACK in data
                if (rxBuf[0] == 0x41 && rxBuf[1] == 0x43 && rxBuf[2] == 0x4B) { // "ACK"
                    Serial.println(F("Slave acknowledged our message!"));
                    received = true;
                    break;
                }
            } else {
                Serial.println(F("(unexpected ID, continuing to wait...)"));
            }
        }
        delay(10);
    }
    ...
    if (!received) {
        ...
        Serial.println(F("\nPossible causes:"));
        Serial.println(F("  1. Slave not in CAN test mode (send 'CAN' to Slave)"));
        Serial.println(F("  2. CANH/CANL not connected between modules"));
        Serial.println(F("  3. Slave CAN module not working"));
        Serial.println(F("  4. Termination resistor issue"));
        return false;
    }
    
    Serial.println(F("\n=== TWO-ARDUINO TEST PASSED ==="));
    Serial.println(F("Master <-> Slave CAN communication working!\n"));
    return true;
}
